...
__device__ unsigned char d_sbox[256] =
{
...
__global__ void aes_encryption_shared(unsigned char *message, unsigned char *result, unsigned char *sbox, unsigned char *keys, unsigned int width)
{
	...
	__shared__ unsigned char sharedKeys[176];
	...
	// Starting AES Rounds
	key_addition(iv_new, sharedKeys, 0);
	...
	for (int i = 1; i != NUM_ROUNDS; ++i)
	{
		...
		shift_rows(iv_new);
		mix_columns(iv_new);
		key_addition(iv_new, sharedKeys, i);
	}

	byte_sub(iv_new, sharedSbox);
	shift_rows(iv_new);
	key_addition(iv_new, sharedKeys, NUM_ROUNDS);
...
// Pre Mix Column 
__device__ void aes_MixColumns(unsigned char *message) 
{
	int i, j;
